...
with col_gauge:
    ...
    st.markdown("""
        <div style="background-color: #1f2630; padding: 10px; border-radius: 8px; border: 1px solid #30363d; font-size: 0.8rem;">
            ...
            <p style="margin-bottom: 5px;">🟡 <b>20% - 35%:</b> Riesgo moderado. Cuidado con nuevos créditos.</p>
            <p style="margin-bottom: 0px;">🔴 <b>+35%:</b> Sobreendeudamiento. Difícil aprobación bancaria.</p>
        </div>
    """, unsafe_allow_html=True)
...
# --- SECCIÓN 3: VALIDACIÓN DE POLÍTICAS ---
st.write("")
...
# --- SECCIÓN 4: ESTRATEGIAS DE OPTIMIZACIÓN ---
st.write("---")
...
with opt_col1:
    ...
    monto_opt1 = int((c_deuda * factor) - prestamo) if int((c_deuda * factor) - prestamo) > 0 else 0
    st.markdown(f'<div class="opt-card opt-blue"><div class="opt-header header-blue">📈 Proyección por Deuda</div><div class="opt-text">Bajando tus tarjetas al 50%, tu techo de inversión sube:</div><div class="opt-monto">+ S/ {monto_opt1:,}</div></div>', unsafe_allow_html=True)
with opt_col2:
    c_proy = int(max(0, ((ingreso + 500) * 0.40) - deudas))
    st.markdown(f'<div class="opt-card opt-blue"><div class="opt-header header-blue">🚀 Proyección por Ingresos</div><div class="opt-text">Si tus ingresos aumentan en <b>S/ 500</b>, tu techo de inversión sube:</div><div class="opt-monto">+ S/ {int((c_proy * factor) - prestamo):,}</div></div>', unsafe_allow_html=True)
with opt_col3:
    n_pct = ((inicial + 5000) / (escenarios[1]['monto'] + 5000)) * 100
    st.markdown(f'<div class="opt-card opt-green"><div class="opt-header header-green">💰 Ahorro Estratégico</div><div class="opt-text">Si ahorras <b>S/ 5,000</b> extra, tu inicial sube a:</div><div class="opt-monto">{n_pct:.2f}%</div></div>', unsafe_allow_html=True)
